...
// Styles
const styles = {
  input: "w-full bg-[#181A20] text-white rounded-lg p-3 border border-[#2B3139] focus:border-[#F0B90B] focus:outline-none focus:ring-1 focus:ring-[#F0B90B]/20 transition-all duration-200 placeholder:text-gray-500",
  select: "appearance-none bg-[#181A20] text-white rounded-lg p-3 border border-[#2B3139] focus:border-[#F0B90B] focus:outline-none focus:ring-1 focus:ring-[#F0B90B]/20 transition-all duration-200",
  button: "w-full bg-[#F0B90B] hover:bg-[#F0B90B]/90 text-black font-medium py-3 px-4 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed",
  modalBg: "bg-[#0B0E11] border-[#2B3139]",
  sectionBg: "bg-[#181A20] border-[#2B3139]",
  text: {
    primary: "text-white",
...
export default function WithdrawalModal({ isOpen, onClose, onHistoryClick }: WithdrawalModalProps) {
  ...
  // Set default network to TRC20 when crypto payment is selected
  useEffect(() => {
    if (isCryptoPayment && !watch('network')) {
      setValue('network', NetworkType.TRC20);
    }
  }, [isCryptoPayment, setValue, watch]);
  ...
  const onSubmit = async (data: WithdrawalFormData) => {
    ...
    const { response , status }  = await API_CALL({ url :'/withdrawals' , method : 'POST' , body : { ...data  }});
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-lg flex items-center justify-center z-50 p-0 sm:p-4 animate-fadeIn overflow-y-auto">
      <div className={`${styles.modalBg} rounded-2xl w-full h-full sm:h-auto sm:max-w-2xl border shadow-2xl transform transition-all duration-300 scale-100 animate-modalSlideIn relative sm:my-4`}>
        {/* Header */}
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-[#2B3139]">
          <div className="flex items-center space-x-4">
            <h2 className={`text-xl font-semibold ${styles.text.primary}`}>Withdraw Crypto</h2>
            {onHistoryClick && (
              <button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-4 sm:p-6 space-y-6">
          {/* Balance Display */}
          <div className={`${styles.sectionBg} rounded-lg p-4`}>
            <div className="flex justify-between items-center mb-2">
              <span className={`text-sm ${styles.text.secondary}`}>Available Balance</span>
              <span className={`text-lg font-medium ${styles.text.primary}`}>${balance.toFixed(3)} USDT</span>
            </div>
                ...
                <div>
                  <div className="relative">
                    <input
                      ...
                      className={`w-full ${styles.sectionBg} rounded-lg p-3 pr-10 border ${errors.recipient ? 'border-red-500' : 'border-[#2B3139]'} focus:border-[#F0B90B] focus:outline-none focus:ring-1 focus:ring-[#F0B90B]/20 transition-all duration-200 placeholder:text-gray-500 ${styles.text.primary}`}
          ...
          <div className={`${styles.sectionBg} rounded-lg p-4 space-y-3`}>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between py-1 border-b border-[#2B3139]">
                ...
                <span className={`${styles.text.primary} font-medium`}>{watch('network')?.toUpperCase() || '-'}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-[#2B3139]">
                <span className={styles.text.secondary}>Estimated Arrival</span>
                <span className={`${styles.text.primary} font-medium`}>{selectedMethod === PaymentMethod.BINANCE && watch('network') ? 
